...
public class Robot implements IRobot{
	 ...
	 * 
	 * @param 	x 
	 * 			X-co�rdinaat van de robot.
	 * 
	 * @param 	y 
	 * 			Y-co�rdinaat van de robot.
	 * 
	 * @param 	direction
	 ...
	 * 
	 * @param 	x
	 * 			X-co�rdinaat van de nieuwe positie van de robot.
	 * 
	 * @param 	y
	 * 			Y-co�rdinaat van de nieuwe positie van de robot.
	 * 
	 * @throws 	IllegalArgumentException
	 * 			De gegeven x- en y-co�rdinaten stellen geen geldige positie voor.
	 * 			|!isValidPosition(x, y)
	 * 
	...
	/**
	 * Methode om de x-co�rdinaat van de robot te verkrijgen.
	 * 
	 * @return 	long
	 * 			X-co�rdinaat van de robot.
	 */
	...
	/**
	 * Methode om de y-co�rdinaat van de robot te verkrijgen.
	 * 
	 * @return 	long
	 * 			Y-co�rdinaat van de robot.
	 */
	...
	public void moveNextTo(Robot target){
		if((this.getEnergy() >= 500 || target.getEnergy() >= 500)){
			...
			altpos = this.getCloseTargetCo�rdinate(target.getX(), target.getY());
			double thisNeededEnergy = this.getEnergyBasedOnTurnsAndMoves(altpos[0], altpos[1]);
			altpos = target.getCloseTargetCo�rdinate(this.getX(), this.getY());
			...
			altpos =  target.getCloseTargetCo�rdinate(target.getX(), this.getY());
			...
			altpos = target.getCloseTargetCo�rdinate(this.getX(), target.getY());
			...
			altpos = this.getCloseTargetCo�rdinate(target.getX(), this.getY());
			...
			altpos = this.getCloseTargetCo�rdinate(this.getX(), target.getY());
			...
			if(this.getX() == target.getX() && this.getY() == target.getY()){
				...
				case 3:
					...
					destination2 = target.getCloseTargetCo�rdinate(target.getX(), this.getY());
				...
				case 4:
					...
					destination2 = target.getCloseTargetCo�rdinate(this.getX(), target.getY());
					this.moveTo(this.getX(), target.getY(), false);
					target.moveTo(destination2[0], destination2[1], false);
					break;
				case 5:
					// Het ideale traject is target naar (target.getX(), this.getY()) bewegen en this ernaast.
					destination2 = this.getCloseTargetCo�rdinate(target.getX(), this.getY());
				...
				case 6:
					...
					destination2 = this.getCloseTargetCo�rdinate(this.getX(), target.getY());
	...
	public double getEnergyRequiredToReach(long xpos, long ypos){
		...
		if(ManhattanAlternative1 < thisNeededEnergy && ManhattanAlternative1 != 0){
			// Effici�nter om Manhattan alternatief 1 te gebruiken (target.getX(), this.getY()).
			return ManhattanAlternative1;
		}else if(ManhattanAlternative2 < thisNeededEnergy && ManhattanAlternative2 != 0){
			// Effici�nter om Manhattan alternatief 2 te gebruiken (this.getX(), target.getY()).
	 ...
	 * 
	 * @param	x 
	 * 			X-co�rdinaat die moet worden nagegaan.
	 * 
	 * @param 	y
	 * 			Y-co�rdinaat die moet worden nagegaan.
	 * 
	 * @return	boolean 
